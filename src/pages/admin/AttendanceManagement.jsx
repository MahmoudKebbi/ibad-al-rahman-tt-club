...
import { getAllAttendanceRecords, getAttendanceStats } from '../../services/firebase/attendance';
...
const AttendanceManagement = () => {
  ...
  const [filters, setFilters] = useState({
    startDate: new Date(new Date().setDate(new Date().getDate() - 7)).toISOString().split('T')[0], // One week ago
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              {dayOfWeekData.map((day) => (
                <div key={day.day}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm text-gray-500">{day.count} visits ({day.percentage}%)</span>
                  </div>
          ...
          <ContentCard title="Date Range Information">
            <div className="bg-gray-50 p-4 rounded-md">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{formatDate(new Date(filters.startDate))}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">End Date</p>
                  <p className="font-medium">{formatDate(new Date(filters.endDate))}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Total Days</p>
                  <p className="font-medium">
                    {Math.ceil((new Date(filters.endDate) - new Date(filters.startDate)) / (1000 * 60 * 60 * 24)) + 1} days
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium">
                    {stats.totalAttendance > 0 
                      ? (stats.totalAttendance / (Math.ceil((new Date(filters.endDate) - new Date(filters.startDate)) / (1000 * 60 * 60 * 24)) + 1)).toFixed(1) 
                      : 0}
                  </p>
                </div>
              </div>
            ...
            <div className="mt-4">
              <h3 className="text-sm font-medium text-gray-700 mb-2">Attendance Overview</h3>
              <div className="w-full bg-gray-200 rounded-full h-2.5 mb-1">
                <div 
                  className="bg-green-500 h-2.5 rounded-full" 
                  style={{ width: `${stats.totalAttendance > 0 ? (stats.checkedIn / stats.totalAttendance) * 100 : 0}%` }}
                ></div>
              </div>
