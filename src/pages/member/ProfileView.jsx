              ...
              <div className="relative">
                <div className="h-32 w-32 rounded-full bg-gray-200 flex items-center justify-center">
                  {profileData?.photoURL ? (
                    <img src={profileData.photoURL} alt={profileData.displayName} className="h-32 w-32 rounded-full object-cover" />
                  ) : (
                ...
                <Link to="/member/profile/photo" className="absolute bottom-0 right-0 bg-white rounded-full p-2 shadow-md hover:bg-gray-100">
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-800">{formatDate(profileData?.createdAt)}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-gray-500">Playing Level</h3>
                  <p className="text-gray-800">{getPlayingLevelDisplay(profileData?.playingLevel)}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <ContentCard title="Membership Information" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <p className="text-gray-800">{getMembershipTypeDisplay(membershipData?.membershipType)}</p>
            </div>
            <div>
              <h3 className="text-sm font-medium text-gray-500">Status</h3>
              <div className="mt-1">
                <StatusBadge 
                  status={membershipData?.status === 'active' ? 'success' : 'error'}
                  text={membershipData?.status === 'active' ? 'Active' : 'Inactive'}
                />
              </div>
            </div>
            <div>
              <h3 className="text-sm font-medium text-gray-500">Valid Until</h3>
              <p className="text-gray-800">{formatDate(membershipData?.expiryDate)}</p>
            </div>
            <div>
              <h3 className="text-sm font-medium text-gray-500">Last Payment</h3>
              <p className="text-gray-800">{formatDate(membershipData?.lastPayment)}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-gray-800">
                {membershipData?.paymentMethod === 'credit_card' ? 'Credit Card' : membershipData?.paymentMethod}
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            ...
            <p className="text-gray-500">
              ...
              <Link to="/member/payments" className="text-green-600 hover:text-green-700 font-medium">
                Payments page
              </Link>.
            </p>
          </div>
        ...
        <ContentCard title="Personal Information" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-gray-800">{formatDate(profileData?.birthDate) || 'Not provided'}</p>
            </div>
            <div>
              <h3 className="text-sm font-medium text-gray-500">Gender</h3>
              <p className="text-gray-800">{getGenderDisplay(profileData?.gender) || 'Not provided'}</p>
            </div>
          </div>
        </ContentCard>
        ...
        <ContentCard title="Emergency Contact" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-gray-800">{profileData?.emergencyContact?.name || 'Not provided'}</p>
            </div>
            <div>
              <h3 className="text-sm font-medium text-gray-500">Contact Phone</h3>
              <p className="text-gray-800">{profileData?.emergencyContact?.phone || 'Not provided'}</p>
            </div>
          </div>
        </ContentCard>
      </div>
