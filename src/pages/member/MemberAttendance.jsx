...
import { getMemberAttendanceRecords, getCurrentAttendance } from '../../services/firebase/attendance';
...
const MemberAttendance = () => {
  ...
  const columns = [
    ...
    },
    {
      title: 'Duration',
      field: 'duration',
      render: (row) => row.checkOutTime ? calculateDuration(row.checkInTime, row.checkOutTime) : '—'
    },
    {
      title: 'Type',
      field: 'attendanceType',
      render: (row) => (
        <span className="capitalize">{row.attendanceType}</span>
      )
    },
    {
      title: 'Status',
      ...
      render: (row) => (
        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
          row.status === AttendanceStatus.CHECKED_IN ? 'bg-green-100 text-green-800' : 
          row.status === AttendanceStatus.CHECKED_OUT ? 'bg-blue-100 text-blue-800' : 
          'bg-red-100 text-red-800'
        }`}>
          {row.status === AttendanceStatus.CHECKED_IN ? 'Checked In' : 
           row.status === AttendanceStatus.CHECKED_OUT ? 'Checked Out' : 
           'No Show'}
        </span>
      )
    }
  ...
  const calculateStats = () => {
    ...
    const completedSessions = attendanceRecords.filter(r => r.status === AttendanceStatus.CHECKED_OUT).length;
    const totalDuration = attendanceRecords.reduce((total, record) => {
      if (record.checkOutTime) {
        const duration = (new Date(record.checkOutTime) - new Date(record.checkInTime)) / (1000 * 60); // minutes
      ...
      <div className="container mx-auto px-4">
        ...
        <ContentCard className="mb-6">
          <div className="flex items-center">
            <div className={`rounded-full p-3 ${
              currentRecord ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'
            }`}>
            ...
            <div className="ml-4">
              <h3 className="font-medium text-gray-800">Current Attendance Status</h3>
              <p className="text-sm text-gray-600 mt-1">
                {currentRecord ? (
                  <>
                    You are currently checked in. Check-in time: {formatTime(currentRecord.checkInTime)} on {formatDate(currentRecord.checkInTime)}
                  </>
                ) : (
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-2xl font-semibold text-gray-800">{stats.completedSessions}</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-2xl font-semibold text-gray-800">{stats.avgDurationFormatted}</p>
              </div>
        ...
        <ContentCard title="Date Range Filter" className="mb-6">
          ...
          <div className="mt-4 text-sm text-gray-500">
            <p>Showing attendance records from {formatDate(new Date(filters.startDate))} to {formatDate(new Date(filters.endDate))}</p>
          </div>
        </ContentCard>
