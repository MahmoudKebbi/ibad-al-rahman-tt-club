  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
        ...
        <p className="mt-1 max-w-2xl text-sm text-gray-500">Your membership payment records.</p>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {payments.map((payment) => (
              <tr key={payment.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {payment.expirationDate ? payment.expirationDate.toLocaleDateString() : 'N/A'}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    payment.expirationDate && getMembershipStatus(payment.expirationDate) === 'active'
                      ? 'bg-green-100 text-green-800'
                      : 'bg-red-100 text-red-800'
                  }`}>
                    {payment.expirationDate ? 
                      getMembershipStatus(payment.expirationDate) === 'active' ? 'Active' : 'Expired'
                      : 'Unknown'}
                  </span>
                </td>
