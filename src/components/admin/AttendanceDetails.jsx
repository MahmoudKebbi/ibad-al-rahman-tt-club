    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const formattedAttendance = {
          ...
          checkOutTime:
            attendanceData.checkOutTime?.toDate() || attendanceData.checkOutTime
              ? new Date(attendanceData.checkOutTime)
              : null,
          createdAt:
            attendanceData.createdAt?.toDate() ||
            new Date(attendanceData.createdAt),
        };
  ...
  const handleCheckOut = async () => {
    try {
      ...
      setAttendance({
        ...
        ...result,
        checkOutTime:
          result.checkOutTime instanceof Date
            ? result.checkOutTime
            : new Date(result.checkOutTime),
      });
          ...
          <ContentCard title="Attendance Details" className="mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium capitalize">
                  {attendance.checkInMethod?.replace("-", " ") || "N/A"}
                </p>
              </div>
            </div>
